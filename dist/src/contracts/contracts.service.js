...
let ContractsService = class ContractsService {
    ...
    async create(userId, createContractDto) {
        ...
        if (project.status === project_status_enum_1.ProjectStatus.OPEN) {
            ...
            await this.projectsService.update(project.id, userId, { status: project_status_enum_1.ProjectStatus.IN_PROGRESS });
        }
    ...
    async update(id, userId, updateContractDto) {
        ...
        if (contract.clientAccepted && contract.freelancerAccepted) {
            ...
            await this.projectsService.update(contract.project.id, userId, { status: project_status_enum_1.ProjectStatus.IN_PROGRESS });
        }
        if (isAdmin && updateContractDto.status) {
            ...
            if (updateContractDto.status === contract_status_enum_1.ContractStatus.COMPLETED) {
                await this.projectsService.update(contract.project.id, userId, { status: project_status_enum_1.ProjectStatus.COMPLETED });
            }
            else if (updateContractDto.status === contract_status_enum_1.ContractStatus.CANCELLED) {
                await this.projectsService.update(contract.project.id, userId, { status: project_status_enum_1.ProjectStatus.CANCELLED });
            }
        }
        return this.contractsRepository.save(contract);
    }
